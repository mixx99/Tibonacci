...
void test_calculating(int number_of_tests = 10, int max_number_of_threads = 5){
    ...
    for(int i = 0; i < number_of_tests; ++i){
        ...
        tib.calculate_fibonacci_numbers_with_threads(rand() % max_number_of_threads + 1);
        for(int j = 0; j < max_size; ++j){
            ...
            if(tib_result != my_result){
                ...
                std::cout << "\tRequired Fibonacci number: " << tib.get_start_number(j) << std::endl;
    ...
    if(errors == 0)
        std::cout << "-----All calculating with threads tests passed successfully-----" << std::endl;
    else
    ...
    if(errors == 0)
        std::cout << "-----All calculating tests without threads passed successfully-----" << std::endl;
    else
        std::cout << "----" << errors << " errors in some tests-----" << std::endl;
}
}