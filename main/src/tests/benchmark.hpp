...
void benchmark(int number_of_elements = 10000, int tests = 10, int number_of_threads = 5){
    ...
    std::cout << "Calculating without threads took: " << t << " seconds." << std::endl;
    simple_gettime(&t1);
    for(int i = 0; i < tests; ++i){
        fill_tib(tib, number_of_elements);
    ...
    std::cout << "Calculating with " << number_of_threads << " threads took " << t << " seconds. " << std::endl;
    std::cout << "Difference is " << std::abs(with_threads_result - without_threads_result) << " seconds." << std::endl; 
}
}